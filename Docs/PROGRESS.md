...
| GĐ 16 | Select "Trường đang xem" ở đầu hub lọc mọi tab; tab đầu thành **Tổng quan trường** (`SchoolOverview`: KPI + thông tin trường + bảng Cơ sở/Nhân sự/Học sinh/Cơ sở vật chất, đọc mock) — bỏ CRUD trường; `CrudManager` thêm `itemFilter` + `createDefaults` (lọc theo `schoolId`, mặc định gắn trường đang chọn khi thêm mới); mock mở rộng 4 trường (1 đầy đủ + 2 gọn: THCS Cái Răng, THCS Bình Thủy, THPT Cái Khế) kèm cơ sở, nhân sự, khối/tổ, lớp, học sinh, học bạ, biến động; thêm mô-đun **Cơ sở vật chất** (`SchoolFacility`, 26 mục kiểm kê); khối 10/11/12 vào catalog Cấp học; Personnel/Sector/Students nhận `schoolId` để scope dữ liệu. Lưu ý: giữ nguyên localStorage key cũ — tab Tổng quan/Select đọc mock trực tiếp, còn tab CRUD hiện trường mới sau khi bấm "Khôi phục mẫu" | `48c4614` | `gd16-school-select-overview-facilities` |
| GĐ 17 | Bản đồ GIS bổ sung **4 cơ sở trường mới** (Cái Răng, Hưng Phú, Bình Thủy, Cái Khế) với `wardId` khớp phường thật trong `cantho_phuongxa_geo.json` → bộ lọc Xã/Phường hoạt động đúng, chọn phường tự động zoom vào ranh giới; dời marker "Phân hiệu Chu Văn An" (campus-002) về **Đại lộ Hòa Bình**; marker **phân hiệu có cảnh báo** (alert `status` = new/processing, bỏ resolved) → icon chớp đỏ + chấm pulse + popup đếm số cảnh báo + dòng legend mới; thêm 2 alert mẫu (phân hiệu Hưng Phú, Chu Văn An); địa chỉ GIS chuẩn hóa **2 cấp** (phường/xã, TP. Cần Thơ — bỏ số nhà/đường, cấp quận); hiệu chỉnh tọa độ Chu Văn An về `(10.033488405660892, 105.78446902819987)` | `9434377` (hiệu chỉnh `77feb1d`) | `gd17-gis-can-bao-phuong` |
