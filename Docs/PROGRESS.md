...
| GĐ 17 | Bản đồ GIS bổ sung **4 cơ sở trường mới** (Cái Răng, Hưng Phú, Bình Thủy, Cái Khế) với `wardId` khớp phường thật trong `cantho_phuongxa_geo.json` → bộ lọc Xã/Phường hoạt động đúng, chọn phường tự động zoom vào ranh giới; dời marker "Phân hiệu Chu Văn An" (campus-002) về **Đại lộ Hòa Bình**; marker **phân hiệu có cảnh báo** (alert `status` = new/processing, bỏ resolved) → icon chớp đỏ + chấm pulse + popup đếm số cảnh báo + dòng legend mới; thêm 2 alert mẫu (phân hiệu Hưng Phú, Chu Văn An) | `9434377` | `gd17-gis-can-bao-phuong` |
